...
- **`/blast/js_stress_example/keystone-arch.html`** — "Keystone": a dry-stone voussoir arch (built by `buildArchScenario` in `blast-stress-solver/scenarios`) resting by contact on two fixed abutments, held up purely by compression. Renders wedge voussoirs as `ConvexGeometry` (per-chunk meshes, not batched — a `BatchedMesh` can't mix the non-indexed convex hulls with the indexed abutment boxes) and shows the live **compression thrust line** via `getSolverDebugLines`. "Knock Out Keystone" drops a heavy striker on the crown so the keystone joints **overstress and fracture** (the only thing that re-partitions the rigid bodies — a bare `cutNodeBonds` desyncs the stress graph but never splits the Rapier body), leaving two unsupported half-arches that collapse. The Load slider, dropped weight, and shots drive the same overstress path. Stands-intact-but-collapses-under-overload is proven headlessly in `arch.showcase.test.ts`.
...
**`cutBond`/`cutNodeBonds` do NOT split Rapier bodies** (important, non-obvious): they only remove bonds from the WASM stress graph + JS tracking (`removedBondIndices`). The Rapier rigid bodies re-partition into separate free-falling pieces *only* via the overstress path — `processOneFracturePass` calls `applyFractureCommands` (which splits bodies exactly along broken bonds) and is gated on `solver.overstressedBondCount() > 0`. A manual cut adds no overstressed bond, so it never triggers a split; the solver graph just desyncs from the (still-unified) Rapier body. To actually make pieces detach and fall you must *overstress* the joints — raise gravity, inject contact force (a projectile/striker, scaled by `contactForceScale`), or set weak limits. This is why `house.collapse.test.ts` collapses the roof with a projectile (not its `cutAllOfTypes` helper), and why the keystone-arch demo knocks the keystone out with a heavy striker rather than cutting it. `cutNodeBonds` is still useful to *stop* a node carrying load (the vehicle demo's impact shedding pairs it with collider disable in `handleNodeDestroyed`).

**Custom per-node geometry** (beyond grid boxes): set `scenario.parameters.fragmentGeometries` to a `THREE.BufferGeometry[]` (one per node; `undefined`/missing entries fall back to a box from `parameters.fragmentSizes`). The core then auto-builds **convex-hull colliders** from each geometry's `position` attribute for non-support nodes (`buildColliderDescForNode`), so you do NOT need to pass `colliderDescForNode` or import RAPIER in a demo. Both Three render paths honor it via `getScenarioFragmentGeometries`. Convention: each geometry must be **centered at its node's centroid** (local origin = centroid), because the adapter places the mesh/collider at the body transform + `baseLocalOffset` (= centroid). `mass === 0` marks a node as a fixed **support** (`isSupport`); a scenario with no mass-0 node is free-floating (the core warns and uses a dynamic root). Connected components with a support land on the fixed root; components without become dynamic bodies. See `archScenario.ts` (pure-data hull corners) + `keystone-arch.ts` (builds `ConvexGeometry` from them) for the pattern, and `scenarioFromFragments.ts` for the fracture-output version.
...
- **`useBatchedMesh: true` requires consistent index-ness across all chunk geometries.** A `THREE.BatchedMesh` throws if you mix indexed and non-indexed geometries, and support nodes always get an *indexed* `BoxGeometry` internally. So a scenario whose `fragmentGeometries` are non-indexed (e.g. `ConvexGeometry`, which has no index) plus any mass-0 support box cannot be batched — use `useBatchedMesh: false` (per-chunk meshes). Non-batched still applies `nodeColors` every frame via `updateChunkMeshes`.
- **The solver's absolute stress scale is geometry-dependent and must be tuned by running** — there is no simple closed form. A cantilever (`cracking.ts`) gets large bond stress from moment/leverage amplification, while an arch (`keystone-arch.ts`) is near `load/area` with no amplification, so the same masses/limits behave very differently. When you can't run the WASM build locally, ship a live Strength/material-scale slider so the user can dial into the colored-but-stable regime, and pin headless tests to *extreme* limits (clearly-strong = intact, clearly-weak = collapse) rather than a calibrated threshold.
