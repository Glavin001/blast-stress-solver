//! Split planning: which children reuse an existing body, which get a new one.
//!
//! Moved out of the Rapier module and generalised over [`BackendHandle`],
//! because none of it is engine-specific — the handle is only ever compared,
//! hashed and sorted. This is the algorithm PhysX lacked: it solves a **global**
//! maximum-overlap assignment, where the C++ adapter picked a local greedy
//! winner per split event. Promoting it here is what lets every engine inherit
//! the better plan rather than reimplementing a worse one.
...
pub struct ExistingBodyState<H: BackendHandle> {
    pub handle: H,
...
pub fn plan_split_migration<H: BackendHandle>(
    bodies: &[ExistingBodyState<H>],
    children: &[SplitChild],
) -> SplitMigrationPlan<H> {
...
pub fn plan_split_migration_with_support<H: BackendHandle>(
    bodies: &[ExistingBodyState<H>],
    children: &[SplitChild],
...
#[cfg(any(test, feature = "bench-support"))]
pub fn plan_split_migration_reference<H: BackendHandle>(
    bodies: &[ExistingBodyState<H>],
    children: &[SplitChild],
    child_support: &[PlannerChildSupport],
) -> SplitMigrationPlan<H> {
...
fn plan_split_migration_inner<H: BackendHandle>(
    bodies: &[ExistingBodyState<H>],
    children: &[SplitChild],
...
fn assign_by_components<H: BackendHandle>(
    bodies: &[ExistingBodyState<H>],
    unmatched_bodies: &[usize],
    children: &[SplitChild],
    unmatched_children: &[usize],
    reuse: &mut Vec<ReuseEntry<H>>,
) -> HashSet<usize> {
...
#[cfg(any(test, feature = "bench-support"))]
fn assign_dense_hungarian<H: BackendHandle>(
    bodies: &[ExistingBodyState<H>],
    unmatched_bodies: &[usize],
    children: &[SplitChild],
    unmatched_children: &[usize],
...
#[cfg(any(test, feature = "bench-support"))]
fn build_overlap_matrix<H: BackendHandle>(
    bodies: &[ExistingBodyState<H>],
    unmatched_bodies: &[usize],
    children: &[SplitChild],
    unmatched_children: &[usize],
...
mod tests {
    ...
    use super::*;
    use crate::backend::OpaqueId;

    /// Stand-in for an engine handle. The planner only compares, hashes and
    /// sorts these, which is exactly why it needed no engine in the first place.
    fn mk_handle(i: usize) -> OpaqueId {
        OpaqueId(i as u64 + 1)
    }

    fn mk_bodies(sets: &[Vec<u32>]) -> Vec<ExistingBodyState<OpaqueId>> {
        sets.iter()
            .enumerate()
            .map(|(_i, nodes)| ExistingBodyState {
                handle: mk_handle(_i),
    ...
    fn plan(bodies: &[ExistingBodyState<OpaqueId>], children: &[SplitChild]) -> SplitMigrationPlan<OpaqueId> {
    ...
    fn plan_ref(bodies: &[ExistingBodyState<OpaqueId>], children: &[SplitChild]) -> SplitMigrationPlan<OpaqueId> {
        plan_split_migration_reference(bodies, children, &support(children))
    }

    fn overlap(body: &ExistingBodyState<OpaqueId>, child: &SplitChild) -> usize {
    ...
    fn validate_and_score(
        p: &SplitMigrationPlan<OpaqueId>,
        bodies: &[ExistingBodyState<OpaqueId>],
        children: &[SplitChild],
    ) -> usize {
        ...
        let mut reuse_count: std::collections::HashMap<OpaqueId, u32> = Default::default();
    ...
    fn brute_optimum(bodies: &[ExistingBodyState<OpaqueId>], children: &[SplitChild]) -> usize {
        fn rec(bi: usize, b: &[ExistingBodyState<OpaqueId>], c: &[SplitChild], used: &mut [bool]) -> usize {
            if bi == b.len() {
                return 0;
            }
    ...
    fn reuse_pairs(p: &SplitMigrationPlan<OpaqueId>) -> Vec<(usize, OpaqueId)> {
        let mut v: Vec<_> = p.reuse.iter().map(|r| (r.child_index, r.body_handle)).collect();
        v.sort_by_key(|(c, _)| *c);
        v
    }
    fn create_set(p: &SplitMigrationPlan<OpaqueId>) -> Vec<usize> {
    ...
    fn cascade_fastpath_equals_reference_and_is_optimal() {
            ...
            let bodies = mk_bodies(&[(0..n).collect()]);
    ...
    fn ties_are_resolved_equally_optimally() {
                let bodies = mk_bodies(&[vec![0, 1, 2, 3]]);
    ...
    fn exact_matches_are_always_reused() {
                let bodies = mk_bodies(&[vec![0, 1, 2], vec![3, 4], vec![5, 6, 7, 8]]);
    ...
    fn scales_to_large_inputs() {
        // 7a. one body -> 8192 shards (fast path). Reuse exactly one, create the rest.
                let bodies = mk_bodies(&[(0..8192).collect()]);
