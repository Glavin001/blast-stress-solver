...
import type { BridgeCore, ChunkData, SolverNode } from './types.js';

interface CreateBridgeCoreOptions {
  runtime: any;
...
export function createBridgeCore({ runtime, world, scenario, gravity = -9.81, strengthScale = 1.0, solverSettings }: CreateBridgeCoreOptions): BridgeCore {
  ...
  scenario.nodes.forEach((node: any, nodeIndex: number) => {
    const coord = scenario.gridCoordinates[nodeIndex];
    if (!coord) return;
    if (coord.iy >= 0) {
  ...
  solver.actors().forEach((actor: any) => actorMap.set(actor.actorIndex, { bodyHandle: bridgeBody.handle }));

  const topLayerNodeIndices = scenario.topColumnNodes.reduce((acc: number[], column: any) => {
    if (!Array.isArray(column)) return acc;
    column.forEach((nodeIndex: any) => { if (Number.isInteger(nodeIndex)) acc.push(nodeIndex); });
    return acc;
  }, []);
...
interface BuildBridgeSharedOptions {
  gravity?: number;
  strengthScale?: number;
}
...
export async function buildBridgeShared(options: BuildBridgeSharedOptions = {}): Promise<BridgeCore> {
  ...
  return createBridgeCore({ 
    runtime, 
    world: world as BridgeCore['world'], 
    scenario, 
