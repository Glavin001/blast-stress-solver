/**
 * Destructible Structures Demo
 *
 * Recreates vibe-city's `/destructible-stress` structure presets on top of the
 * high-level blast-stress-solver/rapier + blast-stress-solver/three APIs:
 *
 *   - "Multi-storey frame tower" — buildFrameTowerScenario
 *   - "Mini concrete hut"        — buildConcreteHutScenario
 *   - "Courtyard bungalow"       — buildCourtyardBungalowScenario
 *
 * Pick a structure from the sidebar, then click the viewport to launch
 * projectiles at it. Each preset is a hollow voxel shell bonded on a regular
 * lattice; per-axis bond-area normalization keeps it standing under gravity
 * until a hit (or a removed support) brings it down.
 */
...
// ── Structure presets ─────────────────────────────────────────
// One entry per vibe-city structure, with a camera framing hint so each preset
// reads well on Reset.

type StructureId = 'frameTower' | 'concreteHut' | 'courtyardBungalow';
...
const STRUCTURES: Record<
  StructureId,
  {
    label: string;
    build: () => ScenarioDesc;
    /** Orbit target + camera distance (auto-tuned from the scenario otherwise). */
    framing?: { target: THREE.Vector3; distance: number };
  }
> = {
  frameTower: {
...
const CONFIG = {
  structure: 'frameTower' as StructureId,
  projectile: {
    radius: 0.4,
    mass: 4_000,
    speed: 55,
...
const profiler = createFrameProfilerOverlay({
  exportName: 'destructible-structures-profile',
  getMeta: () => {
    ...
    return {
      demo: 'destructible-structures',
      config: { structure: CONFIG.structure, solver: { ...CONFIG.solver }, physics: { ...CONFIG.physics } },
      live: { bodies, bonds },
    };
  },
});
...
const recorder = createRecordingOverlay({
  ...
  exportName: 'destructible-structures-recording',
...
function frameScenario(scenario: ScenarioDesc) {
  ...
  for (const n of scenario.nodes) {
    box.expandByPoint(v.set(n.centroid.x, n.centroid.y, n.centroid.z));
  }
  if (box.isEmpty()) return;
  const center = box.getCenter(new THREE.Vector3());
  const size = box.getSize(new THREE.Vector3());
  const radius = Math.max(size.x, size.y, size.z) * 0.5;
  // Lift the orbit target toward the middle of the structure.
...
async function initScene() {
  const preset = STRUCTURES[CONFIG.structure];
  ...
  console.log(
    `[destructible-structures] ${preset.label}: ${scenario.nodes.length} nodes, ${scenario.bonds.length} bonds` +
  ...
  recorder.attach(core, { scenario, meta: { demo: 'destructible-structures', config: CONFIG } });
...
// Structure picker — rebuilds immediately on change.
{
  const select = document.getElementById('cfg-structure') as HTMLSelectElement | null;
  if (select) {
    select.value = CONFIG.structure;
    select.addEventListener('change', () => {
      CONFIG.structure = select.value as StructureId;
      void rebuild();
    });
  }
}
