...
function renderDebugLines(bridge) {
  ...
  if (!bridge.world.getRigidBody(bridge.bodyHandle)) {
    updateDebugLineObject(bridge.debugHelper, [], false);
    return;
  }

  // Each debug-line endpoint is a node centroid in bridge-local space — the same
  // value syncMeshes uses (chunk.localOffset) to place that chunk's mesh. Transform
  // each endpoint by the body that CURRENTLY owns its chunk so the overlay follows
  // fragments onto their own rigid bodies after a split, instead of drawing every
  // line at the single root pose (which left green lines floating where the intact
  // structure used to be). Endpoints that don't resolve fall back to the root body.
