 ...
 * `__m128`/`__m256` specializations through simde + a wasm-simd128 FMA/dp_ps
 * override (see `blast/source/shared/stress_solver/simd/simd.h`).  Without the
 * env var the build keeps shipping the scalar `AngLin6Ops<float>` path that
 * clang autovectorizes with `-O3 -msimd128`.
 *
 * This file is the safety gate either way.  It pins:
 *   1. That the `s_use_simd` flag inside `StressProcessor` reflects the build
 *      (so a misconfigured EMCC_USE_SIMD silently shipping the wrong WASM
 *      surfaces at test time, not in production).
 *   2. That a deterministic gravity scenario converges and produces an impulse
 *      magnitude inside a tolerance band the scalar path also satisfies.  The
 *      whole 403-test suite passes under both builds, but a dedicated check
 *      against an absolute number means a future kernel change that drifts the
 *      SIMD path away from scalar shows up here as a small, reviewable diff
 *      instead of an unrelated end-to-end test cascading.
 *
 * Reference values come from running the same scenario through the scalar
 * build at commit `9d1a3ea` (post-build-optimization, pre-simde).
 */
...
describe('SIMD build wiring', () => {
  it(`StressProcessor.usingSIMD() matches EMCC_USE_SIMD (expect ${EXPECT_SIMD})`, async () => {
    ...
    // `StressProcessor::s_use_simd` — flipped to true by the build flag
    // `-DSTRESS_SOLVER_WASM_SIMD=1`, false otherwise.
...
describe('SIMD vs scalar — deterministic gravity scenario', () => {
  it('falling 1-bond column reaches the same converged impulse within solver tolerance', async () => {
    ...
    // bond carries the static reaction. With these units the converged
    // angular impulse is zero (no torque) and the linear impulse is purely
    // along -Y with magnitude m*g per unit volume.
    ...
    // The bond is unbreakable so no fracture commands should appear.
    expect(solver.generateFractureCommands().fractures.length).toBe(0);

    // The residual error should be tight on both paths (solver tolerance is
    // 1e-3 internally; both kernels hit it within machine precision).
