...
| A city of towers all disintegrate at once | Geometry scaled without re-sizing the section | Load per column scales with FLOOR COUNT. Doubling FLOORS while leaving `COL` alone put a 6-floor tower's facade at safety factor **1.03** and its frame at 2.76 -- every class below band, so any impact cascades. Re-run the gravity load path after ANY geometry change; a taller building needs a bigger section, not stronger concrete. |
